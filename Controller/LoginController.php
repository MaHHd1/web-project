...
class LoginController
{
    ...
    public function signin(User $user)
    {
        ...
        if ($result) {
            ...
            if (password_verify($password, $result['psw'])) {
                ...
                $_SESSION['user_name'] = $result['Uname'];  // Store the username
                $_SESSION['user_phone'] = $result['phone']; // Store the phone number
                $_SESSION['user_location'] = $result['location']; // Store the location
                $_SESSION['user_privilege'] = $result['privilege'];
                $_SESSION['user_image'] = $result['IMG'];
    ...
    public function checkLogin()
    {
        ...
        if ($session_id) {
            ...
            if ($user) {
                ...
                $_SESSION['user_name'] = $user['Uname'];  // Store the username
                $_SESSION['user_phone'] = $user['phone']; // Store the phone number
    ...
    public function addUser(User $user)
    {
        try {
            ...
            $phone = $user->getPhone(); // Get phone number
            $location = $user->getLocation(); // Get location
            ...
            $insertQuery = "INSERT INTO users (mail, psw, Uname, Country, phone, location) VALUES (:mail, :psw, :uname, :country, :phone, :location)";
            ...
            $insertStmt->bindParam(':phone', $phone); // Bind phone number
            $insertStmt->bindParam(':location', $location); // Bind location
    ...
    public function updateProfileImage($id, $imageContent)
    {
        global $pdo;

        // Prepare the SQL statement to update the image field as a BLOB
        $query = "UPDATE users SET IMG = :img WHERE id = :id";
        $stmt = $pdo->prepare($query);

        // Bind the parameters
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
        $stmt->bindParam(':img', $imageContent, PDO::PARAM_LOB); // Use PDO::PARAM_LOB for BLOB data
    ...
    public function updateUserProfile($id, $uname, $country, $phone, $location)
    {
        global $pdo;
        $query = "UPDATE users SET Uname = :uname, Country = :country, phone = :phone, location = :location WHERE id = :id";
        $stmt = $pdo->prepare($query);
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
        $stmt->bindParam(':uname', $uname, PDO::PARAM_STR);
        $stmt->bindParam(':country', $country, PDO::PARAM_STR);
        $stmt->bindParam(':phone', $phone, PDO::PARAM_STR); // Bind phone number
        $stmt->bindParam(':location', $location, PDO::PARAM_STR); // Bind location
    ...
    public function getUserById($userId) {
        $query = "SELECT Uname, Country, Phone, Location, IMG, mail FROM users WHERE id = ?";
        $stmt = $this->db->prepare($query);
        $stmt->execute([$userId]);
    ...
    public function getUserImage($userId) {
        ...
        if ($result && $result['IMG']) {
            // Return image data as base64 encoded string
            return base64_encode($result['IMG']);
        } else {
